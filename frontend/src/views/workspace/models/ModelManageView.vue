...
/* ---- 模型编目：将连通性与默认路由聚焦成一个信号面板 ---- */
.models { height: auto; min-height: 100%; padding: clamp(20px, 3vw, 36px); background: var(--color-bg); }.models__head { position: relative; padding: 20px; align-items: center; margin-bottom: 20px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-surface-raised); box-shadow: var(--shadow-float); overflow: hidden; }.models__head::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: var(--color-lifeline); }.models__title { font-size: clamp(25px, 3vw, 34px); }.models__table-wrap { border-radius: var(--radius-lg); background: var(--color-surface-raised); box-shadow: 0 1px 0 rgba(255,255,255,.35) inset; }.models__table thead th { padding: 12px 16px; background: var(--color-bg-elevated); font-size: var(--text-xs); letter-spacing: var(--tracking-label); text-transform: uppercase; }.models__table tbody td { padding: 14px 16px; }.models__table tbody tr { transition: background var(--transition-fast); }.models__table tbody tr:hover td { background: color-mix(in srgb, var(--color-spectrum-e) 6%, var(--color-surface-raised)); }.models__name { font-size: var(--text-base); }.models__ops .op { min-height: 30px; margin: 2px 4px 2px 0; padding: 0 9px; border-radius: 8px; background: var(--color-surface); font-family: var(--font-sans); font-weight: var(--weight-medium); }.models__ops .op:hover { transform: translateY(-1px); background: var(--color-bg-elevated); }.badge { padding: 3px 8px; }.badge--default { background: var(--color-spectrum-e); color: #fff; }.badge--on { background: var(--color-success-bg); }.field__input:focus { border-color: var(--color-focus); box-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus) 13%, transparent); }.field__pwd-btn { transition: background var(--transition-fast), color var(--transition-fast); }.field__pwd-btn:hover { background: var(--color-bg-elevated); color: var(--color-text); } @media (max-width: 720px) { .models__head { align-items: flex-start; flex-direction: column; }.models__table { min-width: 720px; } }
</style>
